...
function getStrengthColor(bar, pass) {
    ...
    if (pass.length === 0) return "rgba(255,255,255,0.15)";
    if (score === 1) return bar <= 1 ? "#FF0000" : "rgba(255,255,255,0.15)";
    if (score === 2) return bar <= 2 ? "#FF6200" : "rgba(255,255,255,0.15)";
    if (score === 3) return bar <= 3 ? "#FFC14E" : "rgba(255,255,255,0.15)";
...
function validate() {
    const newErrors = {};
    if (!password.trim()) newErrors.password = "أدخل كلمة المرور";
    if (!confirmPass.trim()) newErrors.confirmPass = "أدخل تأكيد كلمة المرور";
    if (password !== confirmPass) newErrors.confirmPass = "كلمة المرور غير متطابقة";
    return newErrors;
}
...
function handleNext() {
    ...
    router.push("/register/details");  // ← details مش verify ✅
}
...
>
    {getStrengthLabel(password)}
        ...
        {/* تأكيد كلمة المرور */}
        <div className="flex flex-col gap-2">
            <label className="text-sm text-[#BFC9C4] text-right">تأكيد كلمة المرور</label>
            <div className="relative w-full">
                <input
                value={confirmPass}
            ...
            {confirmPass && password !== confirmPass && (
                <p className="text-xs text-red-400 text-right m-0">كلمة المرور غير متطابقة  </p>)}
                {confirmPass && password === confirmPass && (
                    <p className="text-xs text-[#94D3C1] text-right m-0">كلمة المرور متطابقة  </p>)}
                    {errors.confirmPass && (
